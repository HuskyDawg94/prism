    ...
    <div style={{ background: COLORS.bg, minHeight: '100vh', color: COLORS.text, fontFamily: '"DM Mono", monospace', fontSize: '13px' }}>
      ...
      <div style={{ maxWidth: '720px', margin: '0 auto', padding: '60px 40px' }}>
        ...
        <Section title="What PRISM does with your data">
          ...
          <p style={{ marginBottom: '8px' }}>
            <span style={{ color: COLORS.text }}>Analysis results and synthesis text</span> are stored only in your browser's localStorage. We do not store your full search history, your analysis outputs, or any personally identifying information on our servers.
          </p>
          <p style={{ marginBottom: '8px' }}>
          ...
          <p>
            <span style={{ color: COLORS.text }}>If you use the optional "Is this right?" feedback buttons</span> shown next to individual findings (e.g. "genuinely novel," "already known," "incorrect"), we store your vote in a database on our servers. This is the one exception to "no database" elsewhere on this page, and it exists to let PRISM improve over time. What's stored per vote: the topic/query string you searched, which analysis module the finding came from, the vote itself, and a random identifier generated by your browser and saved in its localStorage — never an account, name, email, or IP address. There is no free-text field; only the three fixed vote choices exist. This is entirely optional — not clicking these buttons has no effect on anything, and using PRISM without ever voting stores nothing in this database. Votes are used only internally, to recalibrate how confident PRISM's modules are in their own outputs — aggregated vote counts are not shown back to any user in the product.
          </p>
        </Section>
        ...
        <Section title="What we do not do">
          ...
          <p style={{ marginBottom: '8px' }}>We do not store your analysis results on our servers. We do not store your search queries on our servers, except that if you use the optional feedback-voting buttons on a finding, the topic string for that search is stored alongside your vote (see above) — nothing else about that search.</p>
        ...
        <Section title="Data retention">
          All session data — your researcher profile, search queries, papers, and analysis outputs — is stored in your browser's localStorage and persists until you clear it using the "Clear Session" button in PRISM or clear your browser's localStorage manually. PRISM's servers retain no search content, analysis output, or profile data between sessions, with one exception: feedback votes submitted through the optional "Is this right?" buttons (topic string, module, vote, and a random browser-generated identifier — described above) are stored in a database and persist indefinitely, so PRISM can learn from them over time. The only other server-side state is the in-memory per-IP usage counter described above, which resets daily and does not survive a server restart.
        </Section>
